...
function Step(operation, previous, options) {
    this._supervisor = new Supervisor(options);
...
function MultiStep(pipelines, previous, options) {
    ...
    this._lastSteps = pipelines.map(function(pipeline) {
        return execute(pipeline, previous, options);
    });
    this._previous = previous;
}
